...
)
{
    ...
    printf("sel4 test: try init kernel\n");
    ...
    printf("ui_p_reg.start=[%lx]\n", ui_p_reg.start);
    printf("ui_p_reg.end=[%lx]\n", ui_p_reg.end);
    printf("dtb_p_reg.start=[%lx]\n", dtb_p_reg.start);
    printf("dtb_p_reg.end=[%lx]\n", dtb_p_reg.end);
    printf("extra_bi_size_bits=[%lx]\n", extra_bi_size_bits);
    printf("it_v_reg.start=[%lx]\n", it_v_reg.start);
    printf("it_v_reg.end=[%lx]\n", it_v_reg.end);
    ...
    cleanInvalidateL1Caches();
    printf("do not create rootserver thread!!!\n");
#if 0
    ...
    ksNumCPUs = 1;
    /* For current ARM Platform of TEE, cpu 0 will always be the first one 
    to boot ok, and it must not be blocked!!
    */
if(getCurrentCPUIndex()) {
    printf("secondary cpus boot up\n");
...
#define TEESMC_OPTEED_RETURN_ENTRY_DONE 0xbe000000
static word_t invokeSMCCall_tee(word_t arg0, word_t arg1, word_t arg2, word_t arg3)
{

    word_t ret = 0;

    register seL4_Word r0 asm("x0") = arg0;
    register seL4_Word r1 asm("x1") = arg1;
    register seL4_Word r2 asm("x2") = arg2;
    register seL4_Word r3 asm("x3") = arg3;
    register seL4_Word r4 asm("x4") = 0;
    register seL4_Word r5 asm("x5") = 0;
    register seL4_Word r6 asm("x6") = 0;
    register seL4_Word r7 asm("x7") = 0;
    asm volatile("smc #0\n"
                 : "+r"(r0), "+r"(r1), "+r"(r2), "+r"(r3),
                 "+r"(r4), "+r"(r5), "+r"(r6), "+r"(r7)
...
)
{
    ...
    activateThread();
#else
    printf("set idle thread as context\n");
    switchToIdleThread();
    activateThread();
    if(!getCurrentCPUIndex()) {
        printf("back to REE\n");
        printf("arm_vector_table:%p\n", arm_vector_table);
        invokeSMCCall_tee(TEESMC_OPTEED_RETURN_ENTRY_DONE,(word_t)arm_vector_table,0,0);
    }    
    /* typically we should go userspace, but for TEE, we want idle thread
        to be in kernel and always waiting for request for REE.
        so we can not use the interface ‘restore_user_context’, and never comes
        back here. 
        TODO: This behavior is not ideal on SEL4 Kernel, change it in the future
     */
    UNREACHABLE();
#endif
}
