...
void init_boot_vspace(struct image_info *kernel_info)
{
    ...
    //BIT(5) is not set, all of these memorys are secure
    ...
    printf("_boot_pud_down[0]=[%p]\n", (uintptr_t)_boot_pud_down[0]);
    ...
    word_t j = GET_PMD_INDEX(first_vaddr);
    printf("_boot_pmd_up[%d]=[%p]\n", j, (uintptr_t)_boot_pmd_up[j]);
}